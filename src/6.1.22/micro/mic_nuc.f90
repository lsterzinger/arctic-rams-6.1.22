...
Subroutine cldnuc (m1,k1cnuc,k2cnuc,k1dnuc,k2dnuc,ktop,kbot,rv,wp,i,j,dn0)
...
integer :: m1,i,j,k,k1cnuc,k2cnuc,k1dnuc,k2dnuc,ktop,kbot &
   ...
   do k = 2,m1-1
      ...
      if (excessrv > 0.) then
         ...
         if(imbudget >= 1) then
           ...
           xnuccldct(k) = xnuccldct(k) + rcnew * budget_scalet
         endif
   ...
   do acat=1,aerocat
     ...
     if((acat==1)                  .or. &  ! CCN
      ...
      if(ctc==1) then
        ...
        !Adele --- start two micron nucleation
        ...
        vaprccn = concen_tab(acat) * emb0(drop)
        if(vaprccn > excessrv*cldrat) then
         concen_tab(acat) = excessrv*cldrat/emb0(drop)
         vaprccn = excessrv * cldrat
        endif
        ...
        if(iccnlev>=1)then
          ...
          if(imbudget >= 1) then
            ...
            xnuccldct(k) = xnuccldct(k) + concen_tab(acat) * budget_scalet
          endif
...
111       continue
   ...
   if(iccnlev==0) then
     ...
     if(imbudget >= 1) then
       ...
       xnuccldct(k) = xnuccldct(k) &
             + (total_cld_nucc + total_drz_nucc) * budget_scalet
     endif
   endif
...
!##############################################################################
Subroutine icenuc (m1,kc1,kc2,kd1,kd2,k1pnuc,k2pnuc,ngr,rv,dn0,dtlt)
...
integer :: m1,kc1,kc2,kd1,kd2,k1pnuc,k2pnuc,ngr,k,idnc,itc,irhhz,ithz,lcat
...
real :: nin_a,nin_b,nin_c,nin_d,concen_tab,embtemp &
       ,frzc,frzr,tot_in,ifntemp,immerin,immersed
...
! probably do not want sink of ccinp here.
   ...
   if(imbudget >= 1) then
     ...
     xnucicect(k) = xnucicect(k) + (cldnuc + ptotvi) * budget_scalet
   endif
   if(imbudget >= 2) then
     xinuchomrt(k)  = xinuchomrt(k)  + homo_nuc * budget_scalet
     xinuccontrt(k) = xinuccontrt(k) + cont_nuc * budget_scalet
     xinuchomct(k)  = xinuchomct(k)  + cldnuc * budget_scalet
     xinuccontct(k) = xinuccontct(k) + ptotvi * budget_scalet
   endif
...
!************************************************************************
   ...
   elseif(iifn==2) then
     ...
     if (excessrv > 0. .and. tairc(k) < 0.0) then
       ...
       nifn(k) = nin_a * (-tairc(k))**nin_b * (tot_in)**(nin_c*(-tairc(k))+nin_d)
    ...
    do lcat=1,ncat
     if(lcat==1.or.lcat==8.or.lcat==2) then
      ...
      if(tairc(k) < 0.0 .and. immerhx(k,lcat) > mincon) then
       ...
       !Original Demott(2010) formula
       !ifntemp = nin_a*(-tairc(k))**nin_b*(tot_in)**(nin_c*(-tairc(k))+nin_d)
       !Modified Demott(2010) for dust-dominated cases
       !Paul suggested an additional factor of 3 multiplier
   ...
   if(imbudget >= 1) then
     ...
     xnucicect(k) = xnucicect(k) + vapnuc * budget_scalet
   endif
   if(imbudget >= 2) then
     xinucifnrt(k) = xinucifnrt(k) + vapnucr * (1.0 - pcthaze) * budget_scalet
     xinuchazrt(k) = xinuchazrt(k) + vapnucr * pcthaze * budget_scalet
     xinucifnct(k) = xinucifnct(k) + vapnuc * (1.0 - pcthaze) * budget_scalet
     xinuchazct(k) = xinuchazct(k) + vapnuc * pcthaze * budget_scalet
   endif
...
!#######################################################################
SUBROUTINE remove_aero_init(dn0,m1,i,j)
...
!(ICCNLEV=2) instead of ICCNLEV=0. 
acat = 1
drop = 1
cnmhx = 0.
...
111       continue
          ...
          !If either is zero, set both to zero
          if(numdrops==0.0 .or. ccnmass==0.0) then
             numdrops=0.0
             ccnmass=0.0
          endif
