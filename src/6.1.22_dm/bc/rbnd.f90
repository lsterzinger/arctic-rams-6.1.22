...
!
! update_cyclic is called from trsets() for updating scalar vars, and from acoust()
! for updating u, v, p, and w. acoust() runs the small timestep so when in parallel
! mode, update_cyclic() and node_sendlbc()/node_getlbc() are called together. For
! this reason, the isflag values in update_cyclic() and node_*lbc() need to remain in sync.
