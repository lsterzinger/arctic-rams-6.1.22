...
! For now, rams_memory_alloc(), which runs alloc_grid() using sub-domain dims,
! is called before getting to this point. This is done so that buffer sizes
! for mpi passing of sub-domain overlap regions can be calculated.
!
! Also for now, we are forcing mainnum node to call isan_driver() on the
! full domain as a workaround to prevent spending time now to develop
! a parallel implementation of isan_driver().
!
! What this means is that, for now, grid_g has already been allocated
! with mainnum's sub-domain dimensions, which are too small. Then right
! before calling isan_driver(), mainnum has been altered to represent the
! full domain (node_mod vars temporarily changed). In order to get back
! in sycn with the scheme, we need to deallocate grid_g, and reallocate
! using the full domain dimensions. At this point mainnum will have had
! its mmxp and mmyp altered to the full domain sizes so use these instead
! of nnxp and nnyp. Using mmxp and mmyp will allow most of this code to remain
! in place if we ever decide to parallelize isan_driver().
...
! Allocate using sub-domain sizes mm[xy]p 
! (which for today's workaround are set to full domain sizes)
...
!         --------------------------------------------------------
   ...
   if(ivrstage == 1) then
      ...
      if(nfeedvar == 1 .and. nigrids > 1) then
         ...
         do ifm=2,nigrids
            icm=nxtnest(ifm)
            CALL fmrefs1d_isan (ifm,icm,maxsigz,mmzp(ifm)  &
                 ,piref,thref,dnref,rtref)
            CALL fmrefs3d_isan (ifm,icm,mmzp(ifm),mmxp(ifm),mmyp(ifm) &
                  ,mmzp(icm),mmxp(icm),mmyp(icm),maxiz,maxix,maxiy  &
            ...
            CALL fmdn0_isan (ifm,icm,mmzp(ifm),mmxp(ifm),mmyp(ifm) &
                  ,mmxp(icm),mmyp(icm),maxix,maxiy &
                  ,rr_scr1(1),rr_scr2(1)  &
                  ,grid_g(ifm)%topt(1,1),grid_g(icm)%topt(1,1) &
...
histloop: do nnudfl=1,nnudfiles
 ...
 do ngrid=1,nigrids
  is_grids(ngrid)%rr_p(1:mmzp(ngrid),1:mmxp(ngrid),1:mmyp(ngrid)) =  &
      is_grids(ngrid)%rr_p  (1:mmzp(ngrid),1:mmxp(ngrid),1:mmyp(ngrid)) &
     +is_grids(ngrid)%rr_pi0(1:mmzp(ngrid),1:mmxp(ngrid),1:mmyp(ngrid))
 enddo
